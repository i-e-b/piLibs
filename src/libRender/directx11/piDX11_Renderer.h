...
public:
    ...
    bool      Initialize(int id, const void **hwnd, int num, bool disableVSync, bool disableErrors, piRenderReporter *reporter, void *device);
	...
	piRasterState CreateRasterState(bool wireframe, bool cullFace, bool frontFace, bool depthClamp, bool multiSample);
	...
	void SetBlendState(const piRasterState vme);
	void DestroyBlendState(piRasterState vme);

	piDepthState CreateDepthState(bool alphaToCoverage, bool enabled0);
	void SetDepthState(const piRasterState vme);
	void DestroyDepthState(piRasterState vme);

    //--- textures ---
    piTexture CreateTexture( const wchar_t *key, const TextureInfo *info, TextureFilter filter, TextureWrap wrap, float aniso, void *buffer );
    ...
    void      DrawPrimitiveIndexed(PrimitiveType pt, int num, int numInstances, int baseVertex, int baseInstance);
