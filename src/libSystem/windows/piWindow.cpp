...
static LRESULT CALLBACK WndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	if ( uMsg == WM_DEVICECHANGE )
	{
		switch ( wParam )
		{
			...
			case DBT_DEVICEREMOVECOMPLETE:
				me->eventinfo.devicesChanged = 1;
				mgr->mEventinfo.devicesChanged = 1;
				break;
			default:
				break;
		}
	...
	if( uMsg==WM_GESTURE )
    {
		...
		// Copy the location
		me->eventinfo.touch.x = gestureInfo.ptsLocation.x;
		me->eventinfo.touch.y = gestureInfo.ptsLocation.y;

		// Copy the type
		switch(gestureInfo.dwID)
		{
		case GID_BEGIN:
			me->eventinfo.touch.type = TOUCH_BEGIN;
			break;
		case GID_PAN:
			me->eventinfo.touch.type = TOUCH_PAN;
			break;
		case GID_ZOOM:
			me->eventinfo.touch.type = TOUCH_ZOOM;
			break;
		case GID_ROTATE:
			me->eventinfo.touch.type = TOUCH_ROTATE;
			break;
		case GID_TWOFINGERTAP:
			me->eventinfo.touch.type = TOUCH_TWOFINGERTAP;
			break;
		case GID_PRESSANDTAP:
			me->eventinfo.touch.type = TOUCH_PRESSANDTAP;
			break;
		case GID_END:
			me->eventinfo.touch.type = TOUCH_END;
			break;
		default:
			break;
		}
...
static void piWindow_InternalInitTouch( iIqWindow * me )
{
    #if _MSC_VER >= 1700 
    ...
    SetGestureConfig(me->hWnd, 0, 5, gestureConfig, sizeof(GESTURECONFIG));
    #endif
}
...
void piWindow_dumpBuffer( piWindow vme, void *buffer, int xres, int yres, bool fitwindow ) 
{
    ...
    StretchDIBits( me->hDC,0,0,dxres,dyres,
                           0,0, xres, yres,
...
piWindowMessage::MessageResponse piWindowMessage::Prompt(const wchar_t *caption, const wchar_t *message, MessageType type, MessageIcon icon, void * hwnd)
{
    int flags = MB_SETFOREGROUND | MB_APPLMODAL;

    if (type == OK )       flags |= MB_OK;
    if (type == OK_CANCEL) flags |= MB_OKCANCEL;
    if (type == YES_NO)    flags |= MB_YESNO;

    if (icon == EXCLAMATION) flags |= MB_ICONWARNING;
    if (icon == QUESTION)    flags |= MB_ICONQUESTION;
    if (icon == STOP)        flags |= MB_ICONSTOP;

    const int res = MessageBox( (HWND)hwnd, message, caption, flags );
